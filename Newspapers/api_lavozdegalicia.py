...
class LaVozDeGaliciaScraper(NewsScraperBase):
    ...
    def _scrape_list_articles(self, soup, base_url):
        ...
        for article in articles:
            ...
            link = a_tag.get('href', '')
            if link.startswith('/'):
                link = urljoin('https://www.lavozdegalicia.es', link)
            elif not link.startswith('http'):
                continue

            list_author = ''
            auth_card = article.select_one('.author, .autor, .byline, .nota__autor')
            if auth_card:
            ...
            articleid = self.idgen.generate_id_from_url(link) if link else self.idgen.generateshortid('LaVozDeGalicia', datetimestr, final_title)

            ordered = self.article.create_ordered_article(
                'La Voz de Galicia', articleid, datetimestr, details.get('tags', []),
    ...
    def _scrape_article_details(self, soup):
        ...
        # SUBTITLE
        subtitle_elem = (soup.select_one('.subtitle, p.lead') or soup.find('h2'))
        subtitle = self.text.cleantext(subtitle_elem) if subtitle_elem else ''

        # AUTHOR
        author = ''
        auth = soup.select_one('a[rel="author"], .author, .autor, .byline, .firma a')
        if auth:
        ...
        # BODY: fallbacks robustos para La Voz de Galicia
        body_container = (soup.find('div', class_=re.compile(r'(article-body|article__body|cuerpo|articulo|content|article-body__content|articleText)'))
                          or soup.find('article') or soup.find('main'))
        paragraphs = []
        if body_container:
            # Priorizar párrafos con clase o role específica y luego cualquier <p>
            paragraphs = body_container.find_all('p', class_=re.compile(r'(paragraph|p|texto|article-paragraph)')) or body_container.find_all('p')
        ...
        for p in paragraphs:
            ...
            if not t:
                continue
            low = t.lower()
            # evitar eliminar párrafos largos que mencionen 'publicidad' en contexto legítimo
            if (len(t) < 40) and ('publicidad' in low or 'anuncio' in low):
                continue
            # aceptar párrafos por número de palabras para evitar falsos negativos
            if len(t.split()) < 6:
                continue
            body_parts.append(t)
        body = ' '.join(body_parts)[:5000]

        # IMAGE: usar helper y completar credits
...
if __name__ == '__main__':
    ...
    lv = LaVozDeGaliciaScraper()
    app = NewsFlaskApp(lv, "La Voz de Galicia", 5013, ["lavozdegalicia.es"])
    app.run()